...
SUBROUTINE hb_loop(phi,Teff,f,ms,icount,rate,nstep,dx,dy,idummy,Tc,Tbath,dphi,r0,v0,mx,my)
!!
!!-----------------------------------------------------------------------
...
!! Note : comment !f2py intent(in,out) allows python to read inout parameters
  ...
  REAL*8  :: ran2,dfree,Teff1,phi1,df
...
REAL*8 FUNCTION dfree(mx,my,Tc,r0,i,j,phi,phi1,Teff,Teff1,v0,dx,dy)
!!
!!-----------------------------------------------------------------------
...
!!
  IMPLICIT NONE
  INTEGER mx,my,i,j
  REAL*8 r0,phi(mx,my),v0(mx,my),dgrad2,Tc
  REAL*8 Teff(mx,my),phi1,df,dx,dy,Teff1
  df = -0.5*(1.0-v0(i,j))*(phi1**2-phi(i,j)**2)+0.25*(phi1**4-phi(i,j)**4)
  df = df + 0.5*r0*dgrad2(mx,my,dx,dy,i,j,phi,phi1)
  df = df + 0.5*Teff1/Tc*phi1**2 - 0.5*Teff(i,j)/Tc*phi(i,j)**2
  dfree = df*dx*dy
  RETURN
END FUNCTION
