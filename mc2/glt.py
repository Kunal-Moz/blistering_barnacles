import os
os.system("python -m numpy.f2py -c Montecarlo2.f90 -m montecarlo")
...
path_to_file = "Test_Data/hyst_test/"
...
parameter = {
    ...
    'Tc':1.0,       # Critical Temperature (Scaled Value)
    ...
    'tloop':True  # True --> T_bath varies and E is constant , False --> T_bath constant and E is varying
}
...
run_par = {
    ...
    'maxE' : 10.2,    # Max value of Electric Field
    ...
    'seed' : 7378552,  # Random number generator seed
    'NPEs' : 1,       # Number of Processors for MPI ( = 1 if MPI is not used )
    'iphi' : False   # If True --> then initial Phi = 1 , False --> given distribution of phi
}
...
#        dux[:,-1] = 0.5*(-f[:,-2] + f[:,0] )/self.dx
        dux[:,0] = 0.5*(f[:,1] - f[:,0])/self.dx
        dux[:,-1] = 0.5*(-f[:,-2] + f[:,-1] )/self.dx
...
def run_mcloop():
    ...
    if iphi:
        ...
        phit = np.loadtxt("phi_val_1.10_temp.dat")
...
#            if i%200 == 0 :
            if i == nmeas-1:
                if tloop:
                    np.savetxt(path_to_file + "phi_val_" + format(Tbvals[k], '.2f') + "_" + str(i)  + "_.dat", glt.phi)
                else:
                    np.savetxt(path_to_file + "phi_val_" + format(Evals[k], '.2f') + "_" + str(i)  + "_.dat", glt.phi)
        np.savetxt(path_to_file + "t_data_" + str(k) + ".dat",temp_data[:, 6])
        Data_Set[k, 1:11] = np.mean(temp_data, axis=0)
        # Data_Set[k, 8] = np.sqrt(Data_Set[k, 8] - Data_Set[k, 7]** 2)
